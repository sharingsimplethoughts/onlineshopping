...
class SubCategory(models.Model):
	...
	des             = models.TextField(blank=True,null=True)
	def __str__(self):
		return self.category.cat_name+ '-' +self.subcat_name
...
class SubSubCategory(models.Model):
	...
	des             	= models.TextField(blank=True,null=True)
	# def __str__(self):
	# 	return self.category.cat_name+'-'+self.subcategory.subcat_name+ '-'+ self.subsubcat_name
...
class ProductAvailableColourWithSizeQty(models.Model):
	...
	is_active               = models.BooleanField(default=True) # in case of delete
...
class Product(models.Model):
	...
	user_role               = models.CharField(max_length=5, blank=True,null=True)
	...
	### row in case of no varients add 
	
	is_row                  = models.BooleanField(default=True)

	### In case product deleted and it is connected to orders
...
WEARABLE_TYPE = (('1','Topwear'), ('2', 'Bottomwear'), ('3','Fullwear'))
class ProductImageFor3DView(models.Model):
	product = models.ForeignKey(Product, on_delete=models.CASCADE)
	front 	= models.ImageField(upload_to='Product/3DView')
	back 	= models.ImageField(upload_to='Product/3DView')
	colour = models.ForeignKey(ProductAvailableColourWithSizeQty, blank=True, null=True, on_delete=models.CASCADE)
	wearable_type = models.CharField(max_length=20, choices=WEARABLE_TYPE,default='1')

	def __str__(self):
		return self.product.name+'-'+ str(self.colour)
...
class CustomerProductCart(models.Model):
	...
	# in case product deleted and it is connected with other order

	is_product_deleted  = models.BooleanField(default=False)

	# if product is exchanged product
	is_exchange_product = models.BooleanField(default=False) 
